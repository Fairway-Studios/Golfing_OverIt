...
public class InputController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (canSwing) CheckBallHit(velocity);
    ...
    {
        rb.AddForce(moveInput.normalized * controllerSens * moveInput.magnitude);
        ClampToRadius();
    }
    ...
    void HandleMouseMovement()
    {
        ...
        rb.linearVelocity = ((Vector2)playerOrigin.position + dir - rb.position) * mouseSens;
    }
    ...
    void HitBall(GameObject ball, Vector2 velocity)
    {
        ...
        Vector2 launchPosition = ballRb.position;
        Vector2 launchVelocity = ballRb.linearVelocity + (impulse / ballRb.mass);
        ...
        if (cameraController != null)
        {
            ...
            cameraController.OnBallHit(launchPosition, launchVelocity);
        }
    ...
    public void OnPlayerTeleported()
    {
        ...
        foreach (var ball in allBalls)
        {
            if (ball != null && ball.GetOwnerIndex() == playerIndex) { myBall = ball; break; }
        }
        if (myBall == null) return;

        Vector2 ballPos = myBall.transform.position;
        float rad = safeAngleDegrees * Mathf.Deg2Rad;
        Vector2 safePos = ballPos + new Vector2(Mathf.Cos(rad), Mathf.Sin(rad)).normalized * (readyDistance + 2f);
    ...
    void ApplySinglePlayerAnaglyphOverride()
    {
        ...
        foreach (var e in FindObjectsByType<AnaglyphRenderingController>(FindObjectsSortMode.None))
    ...
    public void SetSensitivity(float sensitivity) { controllerSens = BASE_CONTROLLER_SENS * sensitivity; mouseSens = BASE_MOUSE_SENS * sensitivity; }
